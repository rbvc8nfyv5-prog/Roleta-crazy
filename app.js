...
// detecta fechamento do padrão NO FINAL
function fechaPadrao(seq){
 let len=seq.length;

 if(len>=3){
  let a=seq[len-3],b=seq[len-2],c=seq[len-1];
  if(a!==b && a===c) return {tipo:"curto",ultimo:c};
 }
...
function render(){
 ...
 for(let i=0;i<5;i++){
  ...
  // se padrão não ativo, tenta fechar
  if(!e.ativo){
   ...
   // padrão ativo → só falha se cair DENTRO do par quebrando
   if(seq.length>0){
